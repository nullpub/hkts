...
export type GetModifyFn<S, A> = {
  <T, L extends 1>(A: TC.Applicative<T, L>): (f: (a: A) => $<T, [A]>) => (s: S) => $<T, [S]>;
  <T, L extends 2>(A: TC.Applicative<T, L>): <E>(f: (a: A) => $<T, [E, A]>) => (s: S) => $<T, [E, S]>;
  <T, L extends 3>(A: TC.Applicative<T, L>): <R, E>(f: (a: A) => $<T, [R, E, A]>) => (s: S) => $<T, [R, E, S]>;
  <T, L extends 4>(A: TC.Applicative<T, L>): <U, R, E>(f: (a: A) => $<T, [U, R, E, A]>) => (s: S) => $<T, [U, R, E, S]>;
}
...
// deno-fmt-ignore
export const compose = <A, B>(ab: Traversal<A, B>) =>
...
// deno-fmt-ignore
type TraverseFn = {
  <T, L extends 1>(T: TC.Traversable<T, L>): <S, A>(sta: Traversal<S, $<T, [A]>>) => Traversal<S, A>
  <T, L extends 2>(T: TC.Traversable<T, L>): <S, E, A>(sta: Traversal<S, $<T, [E, A]>>) => Traversal<S, A>
  <T, L extends 3>(T: TC.Traversable<T, L>): <S, R, E, A>(sta: Traversal<S, $<T, [R, E, A]>>) => Traversal<S, A>
